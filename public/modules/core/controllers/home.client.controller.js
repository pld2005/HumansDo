...
angular.module('core').controller('HomeController', ['$scope', '$http', 'Socket', 'Authentication',
	function($scope, $http, Socket,  Authentication) {
		...
		Socket.on('message', function(msg, id) {
		    $http.get('/wheel').success(function (data){
				drawWheel(data);
			}).error(function(){
		...
		function drawWheel(json) {
			...
			textEnter.append('tspan')
	  			...
	  			.style('visibility', function(d){
	  				return (d.depth<4) ? 'inline' : 'hidden';
	     		});
				    ...
				    text.style('visibility', function(e) {return isParentOf(d, e) ? null : d3.select(this).style('visibility');})
				        ...
				        .each('end', function(e) {
				          d3.select(this).style('visibility', isParentOf(d, e) ? null : 'hidden');
							if (d.depth!==3){
								if (e.depth===4){
									this.getElementsByTagName('tspan')[0].style.visibility = 'hidden';
								}
							}else{
								if (isParentOf(d,e)) {
									this.getElementsByTagName('tspan')[0].style.visibility = 'visible';
								}
							}
				        });
	  			}
